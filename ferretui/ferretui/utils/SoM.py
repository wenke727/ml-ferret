# 引用自 [OmniParser]，作者：[Yadong Lu]
# 来源链接：[https://huggingface.co/microsoft/OmniParser,]
# 版权所有 © [Yadong Lu/MicroSoft] 2023，遵循 [许可证类型] 许可协议
...
class BoxAnnotator:
    ...
    def __init__(
        ...
        text_scale: float = 0.8,  # 0.8 for mobile/web, 0.3 for desktop # 0.4 for mind2web
...
if __name__ == "__main__":
    image = cv2.imread("../../data/ui/alipay_home.PNG")

    detections = Detections(
        xyxy=np.array([[100, 150, 200, 300], [250, 100, 350, 400]]),  # 示例边界框
        class_id=np.array([0, 0])  # 假设类 0 是 "person"，类 1 是 "cat"
    )

    labels = ["person 0.98", "cat 0.95"]
